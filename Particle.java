...
public class Particle extends Collideable {
	...
	protected double getCollisionTime(Wall other) {
		if (other.isVertical()) {
			...
			if (rightSideTime >= 0 && leftSideTime >= 0) return rightSideTime < leftSideTime ? rightSideTime : leftSideTime;
		} else {
			double bottomSideTime = (other.getY() - _y - _radius)/_vy;
			double topSideTime = (other.getY() - _y + _radius)/_vy;
			if (bottomSideTime >= 0 && topSideTime >= 0) return topSideTime < bottomSideTime ? topSideTime : bottomSideTime;
		}
